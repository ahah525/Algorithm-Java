...
//                cam = nowE;
    ...
    public static int solutin2(int[][] routes) {
        ...
        int answer = 0;
        int prev = -30001;   // 캠이 설치된 위치
        for(int[] route : routes) {
            int start = route[0];
            int end = route[1];
            // 2. 현재 자동차가 이전 카메라 범위에 걸리면, 카메라 설치할 필요X(건너뛰기)
            if(start <= prev) continue;
            // 3. 현재 자동차가 이전 카메라 범위에 걸리지 않으면, 진출시점에 새로 카메라 설치
            prev = end;
            answer++;
        }
        return answer;
