...
/**
 * [문제명] [카카오 인턴] 보석 쇼핑
 * [풀이시간] 1시간 / 1시간 8분(18분 + 50분)
 * [한줄평] 구현을 하면서도 시간초과가 날 것 같다고 생각했는데, 결국 시간초과가 났다. 결국 힌트를 얻어 투포인터로 해결했는데 투포인터는 무조건 복습이 필요하다!! / 결국 스스로 풀기는 했지만 투포인터 문제라는 것을 알고 풀어도 조건을 처리하기가 어려웠다.
 * 1_v1. Set, Map, 2중 for 문 - 완전탐색(실패 - 효율성 테스트 1~5, 7~15 실패)
 * - 시작 인덱스, 끝 인덱스를 바꿔가며 완전 탐색 수행
 * 1_v2. Set, Map, 투포인터(성공)
 * [해결법] 투포인터에서 잊지밀아야할 점은 end = n 이 되기전까지 반복한다는것(종료조건)이고, start++ 와 end++ 시점이 중요하다.
 * 2_v1. HashSet, HashMap, 투포인터(실패 - 정확성 2,3,7~9,12,13,15, 효율성 1~3,5~11,13~14)
 * 2_v2. HashSet, HashMap, 투포인터(성공)
