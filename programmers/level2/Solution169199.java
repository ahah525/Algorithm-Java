...
//    }
    ...
    public int bfs(int sx, int sy) {
        ...
        while(!q.isEmpty()) {
            ...
            for(int i = 0; i < 4; i++) {
                P np = move(p.x, p.y, i);
                // 이미 방문했던 곳이면 패스
                if(visited[np.x][np.y] != 0) continue;
                // 목표지점이면
                if(map[np.x][np.y] == 'G') return visited[p.x][p.y];
                // 이동
                q.add(np);
    ...
    // (x, y)에서 d 방향으로 쭉 이동했을 때, 결과 좌표 조회
    public P move(int x, int y, int d) {
        int nx = x;
        int ny = y;
        while(true) {
            nx += dx[d];
            ny += dy[d];
            // 장애물을 만나거나 범위 밖을 벗어나면
