 ...
 * / 보자마자 정렬을 해서 풀어야겠다는 생각이 떠올랐다. 쉬운 그리디 문제였다.
 * 1_v1. 순열(실패 - 정확성 테스트 2~16 실패, 효율성 테스트 모두 실패)
 * [접근법] A에서는 무조건 1,2,3,,, 순서로 뽑는다고 가정하고 B만 순열으로 경우의 수를 구해서 재귀로 풀었는데 시간초과가 났다.
 * 1_v2. 정렬(성공)
 * [해결법] 두 원소의 곱들의 합이 최소가 되기 위해서는 (큰값 * 작은값)을 해야 한다는 점이다!!
 * 2_v1. 그리디, 정렬(성공)
